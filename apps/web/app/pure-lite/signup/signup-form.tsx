...
export const PureLiteSignupForm = () => {
    ...
    const mutation = useMutation({
        mutationFn: async (values: FanSignupInput) => {
            ...
            const result = await signIn("credentials", { email: values.email, password: values.password, redirect: false, callbackUrl: "/pure-lite/content" });
            if (!result || result.error) throw new Error("登録は成功しましたが、ログインに失敗しました。");
            return result;
        },
        onSuccess: (result) => { setMessage("アカウントが作成されました！"); router.push(result?.url ?? "/pure-lite/content"); }
    });
    ...
    return (
        <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
            <button type="button" onClick={() => signIn("google", { callbackUrl: "/pure-lite/content" })}
            ...
            <label className="block text-sm">
                ...
                <input type="password" placeholder="8文字以上" className={clsx("w-full rounded-2xl border bg-white px-4 py-3 text-sm focus:border-pink-400 focus:outline-none", errors.password ? "border-red-400" : "border-pink-200")} {...register("password")} />
                {errors.password && <p className="mt-1 text-xs text-red-500">{errors.password.message}</p>}
            </label>

            <label className="block text-sm">
                <span className="mb-1 block text-xs font-semibold text-[#2d2a26]/60">パスワード（確認）</span>
                <input type="password" placeholder="もう一度入力" className={clsx("w-full rounded-2xl border bg-white px-4 py-3 text-sm focus:border-pink-400 focus:outline-none", errors.confirmPassword ? "border-red-400" : "border-pink-200")} {...register("confirmPassword")} />
            ...
            <div className="space-y-3">
                <label className="flex items-start gap-3 text-sm text-[#2d2a26]/70">
                    ...
                    <span><Link href="/terms" className="text-pink-500 underline">利用規約</Link>と<Link href="/privacy" className="text-pink-500 underline">プライバシーポリシー</Link>に同意します</span>
                </label>
            ...
            <p className="text-center text-sm text-[#2d2a26]/60">
                すでにアカウントをお持ちですか？ <Link href="/pure-lite/login" className="text-pink-500 underline">ログイン</Link>
            </p>
        </form>
    );
};
