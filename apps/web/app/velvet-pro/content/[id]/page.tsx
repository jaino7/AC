...
export default function VelvetProContentDetailPage() {
    ...
    const { data, isLoading, refetch } = useQuery({
        queryKey: ["content-detail", "velvet-pro", id],
        queryFn: async () => {
    ...
    if (isLoading) {
        return (
            <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-[#1a1612] via-[#2a231d] to-[#1a1612] text-white">
    ...
    if (!data) {
        return (
            <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-[#1a1612] via-[#2a231d] to-[#1a1612] text-white">
                <p className="text-gray-400">コンテンツが見つかりません</p>
            </div>
        );
    }
    ...
    return (
        <div className="min-h-screen bg-gradient-to-b from-[#1a1612] via-[#2a231d] to-[#1a1612] text-white">
            {/* ヘッダー */}
            <header className="border-b border-yellow-900/20 px-6 py-4 bg-black/40 backdrop-blur">
                <div className="mx-auto max-w-5xl">
                    <Link
                        href="/velvet-pro/content"
                        className="inline-flex items-center gap-2 text-sm text-gray-400 hover:text-yellow-500"
                    >
                    ...
                    <div className="mt-4">
                        ...
                        <h1 className="text-3xl font-bold text-white mt-1">{post.title}</h1>
                    </div>
                </div>
            </header>
            ...
            <main className="mx-auto max-w-5xl px-6 py-8">
                <div className="grid gap-8 lg:grid-cols-[1fr,400px]">
                    ...
                    <div className="space-y-6">
                        ...
                        {sampleMedia.length > 0 && (
                            <div className="space-y-4">
                                ...
                                {/* メディア表示 */}
                                <div className="rounded-2xl overflow-hidden bg-black/40 border border-yellow-900/20">
                                        ...
                                        {sampleMedia.map((media, index) => (
                                            <button
                                                ...
                                                className={`flex-shrink-0 w-24 h-24 rounded-xl overflow-hidden border-2 transition ${selectedSampleIndex === index
                                                    ? "border-yellow-500"
                                                    : "border-yellow-900/20 hover:border-yellow-900/40"
                                                    }`}
                                            >
                                                {media.type === "VIDEO" ? (
                        ...
                        {mainMedia.length > 0 && !hasAccess && (
                            <div className="rounded-2xl border border-yellow-600/30 bg-gradient-to-br from-yellow-600/10 to-amber-600/10 p-8 text-center">
                                ...
                                {!isLoggedIn ? (
                                    ...
                                    <button
                                        ...
                                        className="inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-yellow-600 to-yellow-500 px-8 py-3 font-semibold text-black hover:from-yellow-500 hover:to-yellow-400 transition disabled:opacity-50 disabled:cursor-not-allowed"
                                    >
                                        <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid gap-4">
                                    {mainMedia.map((media, index) => (
                                        <div key={media.id} className="rounded-2xl overflow-hidden bg-black/40 border border-yellow-900/20">
                                            {media.type === "VIDEO" ? (
                                                <video
                                                    src={media.url}
                                                    controls
                    ...
                    <aside className="space-y-6">
                        {/* 投稿情報 */}
                        <div className="rounded-2xl border border-yellow-900/20 bg-black/40 p-6">
                            <dl className="space-y-3 text-sm">
                                ...
                                {(() => {
                                    ...
                                    return images.length > 0 && (
                                        <div>
                                            <dt className="text-gray-400">画像</dt>
                                            <dd className="text-white font-medium">{images.length}枚</dd>
                                        </div>
                                    );
                                })()}
                            </dl>
