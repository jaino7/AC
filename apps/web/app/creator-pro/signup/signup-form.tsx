...
export const CreatorProSignupForm = () => {
    ...
    const mutation = useMutation({
        ...
        onSuccess: (result) => { setMessage("アカウントが作成されました！"); router.push(result?.redirectUrl ?? result?.url ?? "/"); }
    });
        ...
        <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
            ...
            <label className="block text-sm">
                ...
                <div className="relative">
                    <input type={showPassword ? "text" : "password"} placeholder="8文字以上" className={clsx("w-full rounded-lg border bg-[#0d1117] px-4 py-3 pr-12 text-sm text-white placeholder:text-white/40 focus:border-cyan-400 focus:outline-none", errors.password ? "border-red-500" : "border-gray-700")} {...register("password")} />
            ...
            <label className="block text-sm">
                ...
                <div className="relative">
                    <input type={showConfirmPassword ? "text" : "password"} placeholder="もう一度入力" className={clsx("w-full rounded-lg border bg-[#0d1117] px-4 py-3 pr-12 text-sm text-white placeholder:text-white/40 focus:border-cyan-400 focus:outline-none", errors.confirmPassword ? "border-red-500" : "border-gray-700")} {...register("confirmPassword")} />
            ...
            <div className="space-y-3">
                <label className="flex items-start gap-3 text-sm text-white/70">
                    ...
                    <span><Link href="/terms/fans" className="text-cyan-400 underline">利用規約</Link>と<Link href="/privacy" className="text-cyan-400 underline">プライバシーポリシー</Link>に同意します</span>
                </label>
