...
type Fan = {
    ...
    hasTwitter: boolean;
    totalSupport: number;
    // Status is only used for internal logic or if needed later, but removed from table as requested
...
const fans: Fan[] = [
    ...
    },
    {
        ...
        purchasedTitles: ["未公開ムービー", "バックステージフォト"],
        purchaseCount: 2,
        lastPurchasedDate: "2024/10/10",
        hasTwitter: false,
        totalSupport: 35000,
        status: "active"
    },
    {
        ...
        plan: null,
        purchasedTitles: ["デジタル写真集 Summer", "メイキング映像"],
        purchaseCount: 2,
        lastPurchasedDate: "2024/12/24",
        hasTwitter: true,
        totalSupport: 1200,
        status: "active"
    }
...
export default function FanManagementPage() {
    ...
    const filteredFans = fans
        .filter(fan => {
            if (activeTab === "plans") {
                // Must have a plan
                if (fan.plan === null) return false;

                // Apply plan filter
                if (planFilter !== "全て" && fan.plan !== planFilter) {
                    return false;
                }
