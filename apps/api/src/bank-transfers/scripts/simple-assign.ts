...
async function assignAccountToCreator(creatorId: string) {
  ...
  try {
    ...
    console.log(`  Email: ${creator.user?.email || 'N/A'}`);
    console.log('');
    ...
    if (!availableAccount) {
      throw new Error(
        'No available virtual accounts. Please import more accounts.',
      );
    }
    ...
    console.log('');
    console.log(`  Bank Name:       GMOあおぞらネット銀行`);
...
if (!creatorId) {
  console.error('Usage: ts-node simple-assign.ts <creatorId>');
  process.exit(1);
}
